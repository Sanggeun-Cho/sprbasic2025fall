...
public class BoardServiceImpl implements BoardService {
    ...
    public defaultDto.CreateResDto create(BoardDto.CreateReqDto param) {
        ...
        defaultDto.CreateResDto res = boardRepository.save(board).toCreateResDto();
        return res;
    }

    @Override
    public void update(BoardDto.UpdateReqDto param) {
        Board board = boardRepository.findById(param.getId())
    ...
    public void delete(BoardDto.UpdateReqDto param) {
        update(BoardDto.UpdateReqDto.builder()
                .id(param.getId())
                .deleted(true)
                .build());
    }
    ...
    public List<BoardDto.DetailResDto> list(BoardDto.ListReqDto param) {
        ...
        for(Board each : list){
            ...
            if(param.getDeleted() != null) {
                if(!each.getDeleted().equals(param.getDeleted())){
                    is_this = false;
                }
            }
            ...
            if(is_this){
                final_list.add(detail(defaultDto.DetailReqDto.builder()
                        .id(each.getId())
                        .build()));
            }
        }
